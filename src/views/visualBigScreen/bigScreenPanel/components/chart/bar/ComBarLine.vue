...
<script lang="ts" setup>
  ...
  import { barLineData } from '../../../tool/chartStaticData';
  ...
  const initData = () => {
    ...
    if (props.chartData && props.chartData.dataType === 1) {
      ...
      if (props.chartData?.staticData?.xData && isArray(props.chartData?.staticData?.xData)) {
        ...
        option.xAxis.data = barLineData.xData;
      }
      ...
      if (props.chartData?.staticData?.series && isArray(props.chartData?.staticData?.series)) {
        ...
        seriesData = barLineData.series as SeriesOption[];
      }
      seriesData.forEach((element) => {
        if (element.type == 'bar') {
          series.push({
            ...
            type: element.type,
            yAxisIndex: 0,
            stack: element.stack ? element.stack : uniqueId('bar_'),
          ...
          series.push({
            ...
            yAxisIndex: 1,
            stack: element.stack ? element.stack : uniqueId('bar_'),
            showSymbol: props.chartConfig?.lineSetting?.showSymbol,
            symbolSize: props.chartConfig?.lineSetting?.symbolSize,
            symbol: props.chartConfig?.lineSetting?.symbol || 'circle',
            smooth: props.chartConfig?.lineSetting?.smooth,
            areaStyle: {
              opacity: props.chartConfig?.lineSetting?.areaOpcity,
            },
            lineStyle: {
              width: props.chartConfig?.lineSetting?.lineWidth || 2,
              type: props.chartConfig?.lineSetting?.lineType || 'solid',
            },
            label: {
              show: props.chartConfig?.lineLabelSetting?.show,
              position: props.chartConfig?.lineLabelSetting?.position || 'top',
              color: props.chartConfig?.lineLabelSetting?.labelColor || '#fff',
            },
          });
        }
      });
  ...
  const barSetting = () => {
    ...
    option.xAxis.name = props.chartConfig?.xSetting?.name || 'x轴';
    option.xAxis.splitLine.show = props.chartConfig?.xSetting?.splitLineShow;

    // y轴
    option.yAxis[0].show = props.chartConfig?.YSetting?.leftShow;
    option.yAxis[0].axisLine.lineStyle.color =
      props.chartConfig?.YSetting?.leftLineColor || '#ffffff';
    option.yAxis[0].name = props.chartConfig?.YSetting?.leftName;
    option.yAxis[1].show = props.chartConfig?.YSetting?.rightShow;
    option.yAxis[1].axisLine.lineStyle.color =
      props.chartConfig?.YSetting?.rightLineColor || '#ffffff';
    option.yAxis[1].name = props.chartConfig?.YSetting?.rightName;

    option.yAxis[0].axisLine.lineStyle.width = props.chartConfig?.YSetting?.lineWidth || 1;
    option.yAxis[1].axisLine.lineStyle.width = props.chartConfig?.YSetting?.lineWidth || 1;
    option.yAxis[0].axisLine.show = props.chartConfig?.YSetting?.axisLineShow;
    option.yAxis[1].axisLine.show = props.chartConfig?.YSetting?.axisLineShow;
    option.yAxis[0].nameTextStyle.color = props.chartConfig?.YSetting?.textColor || '#ffffff';
    option.yAxis[1].nameTextStyle.color = props.chartConfig?.YSetting?.textColor || '#ffffff';
    option.yAxis[0].axisTick.show = props.chartConfig?.YSetting?.axisTickShow;
    option.yAxis[1].axisTick.show = props.chartConfig?.YSetting?.axisTickShow;
    option.yAxis[0].splitLine.show = props.chartConfig?.YSetting?.splitLineShow;
    option.yAxis[1].splitLine.show = props.chartConfig?.YSetting?.splitLineShow;
  };
